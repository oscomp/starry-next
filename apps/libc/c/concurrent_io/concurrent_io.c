...
const int BUFFER_CNT = FILE_SIZE / BUFFER_LEN;

char func(int i, int j) {
    return (i ^ j + 998244353) % 63;
}
...
void writer_process(int writer_id) {
    ...
    // 写入文件内容
    for (int i = writer_id; i < BUFFER_CNT; i += WRITER_COUNT) {
        for (int j = 0; j < BUFFER_LEN; j++) {
            buffer[j] = func(i, j);
        }
        pwrite(fd, buffer, BUFFER_LEN, i * BUFFER_LEN);
    }
...
void reader_process(int reader_id) {
    ...
    for (int round = 0; round < 100; round++) {
        ...
        for (int i = reader_id; i < BUFFER_CNT; i += WRITER_COUNT) {
            ...
            for (int j = 0; j < BUFFER_LEN; j++) {
                if (buffer[j] != func(i, j)) {
                    printf("位置 (%d, %d) 读写不一致：期待 %d, 实际 %d\n", i, j, func(i, j), buffer[j]);
                }
            }
        }
    }
...
int main() {    
    ...
    printf("Starting pread/pwrite consistency test\n");
